...
if ($args.count -ne 3) {
Write-Host "Usage: upgrade-vmware.ps1 <vCenter> <HostList.txt> <profile>"
exit
}
...
Apply-VMHostProfile -Entity $ESXiServer -Profile $Profile -Confirm:$false
Test-VMHostProfileCompliance -VMHost $Profile
RebootESXiServer($ESXiServer)
}
